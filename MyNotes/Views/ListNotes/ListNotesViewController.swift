...
class ListNotesViewController: UIViewController {
    ...
    private var allNotes: [Note] = [] {
        didSet {
            notesCountLbl.text = "\(allNotes.count) \(allNotes.count == 0 ? "Заметок" : "Заметки")"
    ...
    override func viewDidLoad() {
        ...
        if allNotes.count == 0 {
        CoreDataManager.shared.dummyNote() // проверяем есть ли заметки, если пусто - создаем dummy заметку
        }
    }
